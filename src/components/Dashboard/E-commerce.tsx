...
const ECommerce: React.FC<{
  ...
  const { data: userData } = useQuery(
    [queryKeys.getUsers, Limit, Page, filters],
    () => ListUsers({ Role: "USER" })
  );
  const metaUser = (userData?.data.Meta as Meta) || [];

  const { isLoading: bookingisLoading, data: bookingStats } = useQuery([queryKeys.getUserBookings], () =>
    getBookings({})
  );
  const metaBookings = (bookingStats?.data.Meta as Meta) || [];
    ...
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats
          ...
          total={metaBookings?.TotalCount?.toString()}
          rate="0.43%"
          levelUp
        >
        ...
        <CardDataStats
          title="Total Users"
          total={metaUser?.TotalCount?.toString()}
          rate="2.59%"
          levelUp
        >
